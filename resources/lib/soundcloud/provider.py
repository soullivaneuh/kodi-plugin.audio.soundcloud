...
class Provider(kodion.AbstractProvider):
    ...
    def __init__(self):
        ...
        self._client = None
        self._local_map = {'soundcloud.explore': 30500,
    ...
    def _play(self, context, re_match):
        ...
        if not track_id:
            raise kodion.KodimonException('Missing if for audio file')

        json_data = self.get_client(context).get_track_url(track_id)
        location = json_data.get('location')
        if not location:
            raise kodion.KodimonException("Could not get url for trask '%s'" % track_id)

        item = AudioItem(track_id, location)
        return item
    ...
    def _do_collection(self, context, json_data, path, params):
