...
__RE_MATCH_DATETIME__ = re.compile(
    r'^(?P<year>[0-9]{4})[-]?(?P<month>[0-9]{2})[-]?(?P<day>[0-9]{2})[" ""T"](?P<hour>[0-9]{2})[:]?(?P<minute>[0-9]{2})[:]?(?P<second>[0-9]{2})')
...
def parse(datetime_string):
    ...
    if time_only_match:
        return datetime.time(hour=_to_int(time_only_match.group('hour')),
    ...
    if date_only_match:
        return datetime.date(_to_int(date_only_match.group('year')),
    ...
    if date_time_match:
        return datetime.datetime(_to_int(date_time_match.group('year')),
    ...
    if period_match:
        return datetime.timedelta(hours=_to_int(period_match.group('hours')),
    ...
    if abbreviated_match:
        ...
        return datetime.datetime(year=_to_int(abbreviated_match.group('year')),
